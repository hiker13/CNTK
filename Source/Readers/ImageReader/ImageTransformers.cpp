...
ScaleTransformer::ScaleTransformer(const ConfigParameters& config) : ImageTransformerBase(config)
{
    ...
    if (interpolation == "nearest")      m_interp = cv::INTER_NEAREST;
    else if (interpolation == "cubic")   m_interp = cv::INTER_CUBIC;
    else if (interpolation == "lanczos") m_interp = cv::INTER_LANCZOS4;
    else if (interpolation == "linear")  m_interp = cv::INTER_LINEAR;
